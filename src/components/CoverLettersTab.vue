...
const handleCreateCl = async () => {
  ...
  try {
    const clId = await clStore.createNewCoverLetter(
      newClName.value,
      newClCategory.value,
      ''
    );
...
<template>
  <div class="resumes-container">
    ...
    <transition name="slide-down">
      <div v-if="showNewClForm" class="form-card">
        ...
        <div class="form-grid">
          <div class="form-group">
            ...
            <input 
              v-model="newClName" 
              type="text" 
              placeholder="e.g., Senior Full-Stack CL"
              class="form-input"
            />
          </div>
          <div class="form-group">
    ...
    <div v-if="clStore.isLoading" class="loading-state">
      Accessing blueprint vault...
    </div>

    <div v-else-if="clStore.coverLetters.length === 0" class="empty-state">
      <Mail :size="48" class="empty-icon" />
      <h3>No templates found</h3>
      <p>Create your first base cover letter to start tailoring.</p>
    ...
    <div v-else class="resumes-grid">
      ...
      >
        ...
        <h3 class="resume-name">{{ cl.name }}</h3>
